        ...
        char *buffer)
{
  ...
  /*
   * In sysfs you are supposed to give little information and all in one
   * go. A small quote from
   * https://www.kernel.org/doc/Documentation/filesystems/sysfs.txt:
   *  Mixing Types, expressing multiple lines of data, and doing fancy
   * formatting of data is heavily frowned upon.
   *
   * In contrary to both procfs and device nodes, the show function will
   * only be called once, so there's no mechanism needed to show we're
   * done.
   *
   * The return value of this function is the number of bytes we've
   * written into buffer, which is exactly what sprintf returns.
   */
  return sprintf(buffer, "HelloWorld from Rick!");
}
...
int __init sysfs_init(void)
{
   ...
   * Since we use kernel_kobj as parameter, the directory is placed in
   * /sys/kernel. Other options are:
   * - mm_kobj  for /sys/kernel/mm/...
   * - hypervisor_kobj  for /sys/hypervisor/...
   * - power_kobj  for /sys/power/...  (which doesn't seem to work on my
   *   machine)

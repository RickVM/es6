...
static inline __s32 i2c_smbus_access(
            int file, char read_write, __u8 command, int size, union i2c_smbus_data* data)
{
...
i2c_smbus_write_i2c_block_data(int file, __u8 command, __u8 length, __u8* values)
{
    ...
    return i2c_smbus_access(
                file, I2C_SMBUS_WRITE, command, I2C_SMBUS_I2C_BLOCK_BROKEN, &data);
}
...
i2c_smbus_block_process_call(int file, __u8 command, __u8 length, __u8* values)
{
    ...
    data.block[0] = length;
    if (i2c_smbus_access(
                    file, I2C_SMBUS_WRITE, command, I2C_SMBUS_BLOCK_PROC_CALL, &data))
        return -1;
    else
    {
        for (i = 1; i <= data.block[0]; i++)

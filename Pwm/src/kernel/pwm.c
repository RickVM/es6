...
static int __init pwm_init(void) {
  ...
  pwmDevice =
      device_create(pwmClass, NULL, MKDEV(majorNumber, 0), NULL, DEVICE_NAME);
  if (IS_ERR(pwmDevice)) {
...
/*
    This function is called whenever the device is being written to from the
   user space. The data is copied to the message[] buffer together with the
   lenght
*/
static ssize_t dev_write(struct file *filep, const char *buffer, size_t len,
                         loff_t *offset) {
  printk(KERN_INFO "PWM: Device has been written to for minor num: %i",
         minor_num);
  //    char temp[len];
...
// This function is called whenever the device is closed/released by the
// userspace progran
